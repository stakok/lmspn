...
int main(int argc, char* argv[]){
	...
	int batchSize = reader.GetInteger("NetworkParam", "BatchSize", 0);
        int vocabSize = reader.GetInteger("NetworkParam", "vocabSize", 10000);
	...
	int GPUID = reader.GetInteger("NetworkParam", "GPUID", 0);
        ...
        int k_power = reader.GetInteger("NetworkParam", "K_P", 0);
	...
	SPN spn(in_dim, out_dim);
        ...
        spn.readTrainDataIdx(train_file);
        spn.readValidDataIdx(valid_file);
        spn.readTestDataIdx(test_file);

	spn.setWeightsFile(weightsFile);
	spn.setBatchSize(batchSize);
        spn.setVocabSize(vocabSize);
