...
cout << "end..." << endl;
		...
		cout << "Testing Perplexity information: " << prob << "/" << sizeOfTestData << endl;
		cout << "Execution time for testing perplexity: " << (double)(clock()-start_time_a_pass)/CLOCKS_PER_SEC << endl; 
		if(SILENT==0) end_timing("[TIM] End of tick.");
		return (double) exp(-prob/sizeOfTestData);
	}
	...
	double calculateValidationPerplexity(){
		...
		cout << "Execution time for validation perplexity: " << (double)(clock()-start_time_a_pass)/CLOCKS_PER_SEC << endl;   
		if(SILENT==0) end_timing("[TIM] End of tick.");
	...
	double calculatePerplexity(){
		...
		cout << "Execution time for training perplexity: " << (double)(clock()-start_time_a_pass)/CLOCKS_PER_SEC << endl; 
		if(SILENT==0) end_timing("[TIM] End of tick.");
	...
	void copyBatchNgramFromHostToDevice(int mode, int batch_idx){
		...
		for(int i=0;i<GRAM_NUM-1;i++){
			for(int j=0;j<proj_dim;j++){
				for(int k=0;k<batch_samples.size();k++){
					v_out0[(proj_dim*i + j)*sizeOfBatchBlock + k] = INPUT_SCALE * projW[j*voc_size + LM_data.at(batch_samples[k]+i)];
				}
			}
		}
		...
		device_v_out.at(0) = v_out0;*/
                ...
                /*for(int i=0;i<min((int)inputDim, 5);i++){
                        for(int j=0;j<min((int)sizeOfBatchBlock, 5);j++){
                                cout << v_out0[i*sizeOfBatchBlock + j] << " ";
                        }
                        cout << endl;
                }*/
...
cout << scientific << "layer " << l_idx << "(" << min_vle << "," << max_vle << "," << mean_vle << ")" << endl;
for(int i=0;i<min((int)numOfNodes.at(l_idx), 5);i++){
        for(int j=0;j<min((int)sizeOfBatchBlock, 5);j++){
                cout << scientific << device_v_out.at(l_idx)[i*sizeOfBatchBlock + j] << " ";
        }
        cout << endl;
}
}*/
...
/*
				host_vector<float> previous;
				host_vector<float> next;
				previous = device_v_out.at(l_idx-1);
				next = device_v_out.at(l_idx);
				
				for(int i=0;i<numOfNodes.at(l_idx);i++){
					for(int j=0;j<sizeOfBatchBlock;j++){
						next[i*sizeOfBatchBlock+j] = 1;
						int base = 1;
						for(int k=0;k<K;k++){
							base *= (k+1);
							next[i*sizeOfBatchBlock+j] += previous[(i*K+k)*sizeOfBatchBlock+j]/base;
						}
					}
				}
				device_v_out.at(l_idx) = next;
*/


/*host_vector<float> tmp;
tmp = device_v_out.at(l_idx);
double max_vle = tmp[0], min_vle = tmp[0];
double mean_vle = 0;
for(int i=0;i<device_v_out.at(l_idx).size();i++){
	if(tmp[i]>max_vle) max_vle = tmp[i];
	if(tmp[i]<min_vle) min_vle = tmp[i];
...
mean_vle /= device_v_out.at(l_idx).size();
cout << "layer " << l_idx << "(" << min_vle << "," << max_vle << "," << mean_vle << ")" << endl;
for(int i=0;i<min((int)numOfNodes.at(l_idx), 5);i++){
	for(int j=0;j<min((int)sizeOfBatchBlock, 5);j++){
		cout << scientific << device_v_out.at(l_idx)[i*sizeOfBatchBlock + j] << " ";
	}
...
/*cout << "Weight " << l_idx << "(" << min_vle << "," << max_vle << ")" << endl;
if(typeOfLayer[l_idx]==TYPE_SUM_LAYER){
    for(int i=0;i<min((int)numOfNodes.at(l_idx),1);i++){
            for(int j=0;j<min((int)numOfNodes.at(l_idx-1), 12);j++){
                    cout << device_W.at(l_idx-1)[i*numOfNodes.at(l_idx-1) + j] << " ";
            }
            cout << endl;
    }
}*/
...
if(typeOfLayer[l_idx]==TYPE_SUM_LAYER){
    for(int i=0;i<min((int)numOfNodes.at(l_idx),1);i++){
            for(int j=0;j<numOfNodes.at(l_idx-1);j++){
                    cout << scientific << device_W.at(l_idx-1)[i*numOfNodes.at(l_idx-1) + j] << " ";
            }
            cout << endl;
    }
}
...
float tmp1 = 0;
for(int i=0;i<numOfNodes.at(0);i++){
    tmp1 += device_W.at(0)[1*numOfNodes.at(0) + i] * device_v_out.at(0)[i*sizeOfBatchBlock + 0];
}
cout << tmp1 << endl;
exit(0);*/
			if(SILENT==0) end_timing("[TIM] 	End of tick.");
...
*/
	...
	void readTextData(string filename, bool end_notation){ 
        ...
        void readTrainDataIdx(string filename){ 
		string line;
		ifstream myfile(filename.c_str());
	...
	void readValidTextData(string filename, bool end_notation){ 
        ...
        void readValidDataIdx(string filename){ 
		string line;
		ifstream myfile(filename.c_str());
        ...
        void readTestDataIdx(string filename){ 
		string line;
		ifstream myfile(filename.c_str());
	...
	void setBatchSize(int sizeOfBatchBlock){ this->sizeOfBatchBlock = sizeOfBatchBlock; }
        ...
        void setVocabSize(int vocabSize){
